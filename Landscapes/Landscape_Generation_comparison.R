...
## UnVaried
load("Data/UnvariedIsotopeData/posterior_early.csv")
load("Data/UnvariedIsotopeData/posterior_late.RData")
load("Data/UnvariedIsotopeData/PreData/posterior_pre.RData")
...
## Unvaried
posterior.pre.un = posterior.pre.un %>%
  rename("group" = "species") %>% 
  left_join(legend) %>%
  mutate(community = 4) %>%
  mutate(species = species.pre) %>%
  select(-species.pre)
posterior.early.un = posterior.early.unvaried %>%
  rename("group" = "species") %>% 
  left_join(legend) %>%
  mutate(community = 5) %>%
  select(-species.pre)
posterior.late.un = posterior.late.un %>%
  rename("group" = "species") %>% 
  left_join(legend) %>%
  mutate(community = 6) %>%
  select(-species.pre)
...
, posterior.pre.un, posterior.early.un, posterior.late.un)
...
posterior %>% 
  ...
  scale_x_discrete(labels = c("1" = "Pre","2" = "Early", "3" = "Late", "4" = "Pre.un", "5"= "Early.un", "6" = "Late.un")) 
...
ellip.mean.filtered.comp = ellip.mean %>% 
  na.omit() %>%
  filter_ellip.data(., 10, 6) 
save(file = "Data/MeanEllipsesComp.RData", ellip.mean.filtered.comp)
load(file = "Data/MeanEllipsesComp.RData")
# Facet wrap heat maps to see individual ellipses
ellip.mean.filtered.comp %>% 
...
ellip = ellip.data(xy_length, n.posts, 10,6)
...
filtered_data= filter_ellip.data(ellip, n_species,6)

## Save .RData files
save(file = "Data/unfiltered_ellipsesComp.RData", ellip)
save(file = "Data/filtered_ellipsesComp.RData", filtered_data)
