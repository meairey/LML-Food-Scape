...
zero.aggregate = read_file("Working scripts/Models/zero_inflated_aggregate")
...
data_list <- list(
  ...
  # Covariates
  #x = covariate_x, # for early model
  #z_covariate =  matrix(step_a$z_covariate, nrow = length(unique(step_a$SITE)), ncol = length(unique(step_a$YEAR))),
